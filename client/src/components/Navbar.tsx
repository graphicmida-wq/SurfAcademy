...
import { Moon, Sun, Menu, X, User, LogOut, BookOpen, ChevronDown, HelpCircle, Video } from "lucide-react";
            ...
            {isAuthenticated && (
              <Link 
                ...
                className={`px-3 py-2 text-sm font-medium rounded-md transition-colors hover-elevate active-elevate-2 flex items-center gap-1 ${
                  !isScrolled ? "text-white/90 hover:text-white" : location === "/masterclass" ? "text-primary" : "text-foreground/80 hover:text-foreground"
                }`}
                data-testid="link-masterclass"
              >
              ...
              {isAuthenticated && (
                <Link 
                  ...
                  className={`px-3 py-2 text-base font-medium rounded-md transition-colors hover-elevate active-elevate-2 flex items-center gap-2 ${
                    location === "/masterclass" ? "text-primary bg-primary/10" : "text-foreground/80"
                  }`}
                  onClick={() => setMobileMenuOpen(false)}
                  data-testid="mobile-link-masterclass"
                >
                  <Video className="h-4 w-4" />
                  Masterclass
                </Link>
